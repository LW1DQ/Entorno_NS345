#!/bin/bash
# =============================================================================
# Script de Instalación de Herramientas para Investigación en Redes
# Grupo de Investigación - NS-3 y Herramientas de Análisis
# =============================================================================
...
show_banner() {
    echo -e "${CYAN}"
    echo "============================================================================="
    echo "  INSTALACIÓN DE HERRAMIENTAS PARA INVESTIGACIÓN EN REDES"
    echo "  NS-3, Herramientas de Análisis y Entorno de Desarrollo"
    echo "============================================================================="
    echo -e "${NC}"
}
...
install_5g_lena_module_inline() {
    ...
    # Seleccionar la rama compatible con NS-3 3.45
    log_info "Seleccionando rama compatible 5g-lena-v4.1.y..."
    if git checkout -b 5g-lena-v4.1.y origin/5g-lena-v4.1.y; then
        log_message "✓ Rama 5g-lena-v4.1.y seleccionada"
    else
        log_error "Error seleccionando rama 5g-lena-v4.1.y"
        cd ../..
        return 1
    fi
...
install_ns3() {
    ...
    if [[ -d "$NS3_DIR" ]]; then
        log_warning "Directorio NS-3 ya existe: $NS3_DIR"
    ...
    # Descargar NS-3
    log_info "Descargando NS-3 3.45..."
    cd "$HOME"
    ...
    # Agregar al PATH
    echo 'export PATH=$PATH:$HOME/ns-3.45' >> ~/.bashrc
    export PATH=$PATH:$HOME/ns-3.45
...
verify_installation() {
    ...
    if command -v ns3 &> /dev/null; then
        ...
        if [[ -n "$NS3_VERSION" ]] && [[ ! "$NS3_VERSION" =~ "disabled" ]]; then
            ...
            log_message "✓ NS-3 encontrado en PATH (versión 3.45)"
        fi
    else
        # Buscar NS-3 en ubicaciones comunes
        NS3_PATHS=(
            "/usr/local/bin/ns3"
            "/opt/ns-3.45/build/ns3"
            "$HOME/ns-3.45/ns3"
            "$HOME/ns-3.45/build/ns3"
            "./ns-3.45/ns3"
            "./ns-3.45/build/ns3"
        )
...
show_summary() {
    ...
    echo -e "${GREEN}✓ NS-3 3.45 instalado y configurado${NC}"
...
main() {
    ...
    if [[ ! -d "$HOME/ns-3.45/contrib/nr" ]]; then
